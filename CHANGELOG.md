...
- **Fixed** a memory leak (leading to "maximum call stack" error) occurs while processing subject hierarchy.
- **Fixed** an issue where subject validation would incorrectly return `true` in a specific case.
...
- **Breaking-Change**: Cross subject inheritance is no more allowed. Fixes [issue #18](https://github.com/onury/accesscontrol/issues/18).
...
- **Fixed** an issue where deeper inherited subjects (more than 1 level) would not be taken into account while querying for permissions. Fixes [issue #17](https://github.com/onury/accesscontrol/issues/17).
...
- **Fixed** an issue with unioned attributes (when a subject extends another and attributes (globs) are unioned for querying permissions). Fixes [issue #19](https://github.com/onury/accesscontrol/issues/19) ([Notation issue #3][notation-issue-3]).
...
- **Revised** `AccessControl` to throw if any reserved keywords are used (i.e. for subject, resource names) such as `"$"`, `"$extend"`.
...
- **Fixed** a bug where checking permission with multiple subjects would mutate the permission attributes. Fixes [issue #2](https://github.com/onury/accesscontrol/issues/2).
- **Fixed** a mutation issue when an access definition object (`IAccessInfo` instead of subject(s)) passed to `.grant()` or `.deny()` methods.
- **Improvement**: You could grant permissions for multiple subjects at once. Now, you can also grant permissions for multiple resources at the same time. This is very handy when you permit _all attributes_ of the resources; e.g. `ac.grant(['admin', 'superadmin']).readAny(['account', 'video'], ['*'])`. The caveat is that the resources (most probably) have different attributes; so you can either permit all, or only common attributes (e.g. `['id', 'name']`).
- **Improvement**: Extending a subject with a non-existent subject will now throw.
